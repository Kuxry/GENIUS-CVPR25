...
# 是否在加载后冻结（训练阶段建议全 False；纯推理时可 True）
FREEZE_SETHEAD_AFTER_LOAD = False
FREEZE_SEQ_AFTER_LOAD     = False

# 解码阶段的 set 先导偏置强度（LogitsProcessor）；可网格 0.1~1.0
PRIOR_SCALE = 0.1

# ==== Joint loss weights ====
# λ_seq * L_seq + λ_set * L_set
LAMBDA_SEQ = 1.0
LAMBDA_SET = 1.0
...
class T5ForGenerativeRetrieval(nn.Module):
    ...
    def __init__(self, config=None, tokenizer=None, clip_model=None, new_tokenizer=True, init_rq_codebook=True):
        ...
        # 注册原型（必须在这里调用）
          # === 硬编码：加载 seq + set 先验（联合训练时默认不冻结）===
        self._load_seq_prior_hardcoded(freeze=FREEZE_SEQ_AFTER_LOAD)
        self._load_set_head_prior_hardcoded(freeze=FREEZE_SETHEAD_AFTER_LOAD)
        print(f"[Prior] seq/set loaded (hardcoded). prior_scale={PRIOR_SCALE}, "
                f"freeze_seq={FREEZE_SEQ_AFTER_LOAD}, freeze_set={FREEZE_SETHEAD_AFTER_LOAD}")

    def _load_set_head_prior_hardcoded(self, freeze: bool = FREEZE_SETHEAD_AFTER_LOAD):
    ...
    def _load_seq_prior_hardcoded(self, freeze: bool = FREEZE_SEQ_AFTER_LOAD):
        """
        仅从 ckpt 里提取 id_generator.*（T5 生成头）相关权重进行加载。
        注意：如果你的 ckpt 是完整模型，这里会自动过滤只保留 id_generator.*。
        """
        path = PRIOR_SEQ_CKPT
        if not path or not os.path.isfile(path):
            print(f"[Prior|seq] skip: file not found -> {path}")
            return
        sd = torch.load(path, map_location="cpu")
        sd = sd.get("model", sd)
        # 只取 id_generator.*；避免把别的模块覆盖掉
        keep = {k: v for k, v in sd.items() if k.startswith("id_generator.")}
        if not keep:
            # 如果 ckpt 里是 T5 的裸权重（没有前缀），尝试直接把键映射到 id_generator.*
            keep = {
                f"id_generator.{k}": v
                for k, v in sd.items()
                if ("lm_head" in k or "shared" in k or "encoder" in k or "decoder" in k)
            }
        missing, unexpected = self.load_state_dict(keep, strict=False)
        print(f"[Prior|seq] loaded from {path} | missing={len(missing)} unexpected={len(unexpected)}")
        if freeze:
    ...
    def compute_single_batch(self, batch, gpu_id=None):
        ...
        # === SeqHead 交叉熵（忽略 pad=-100）===
        loss_seq = self.criterion(
        ...
        # 额外给出 seq 的监控指标（token 准确率可选）
        with torch.no_grad():
            # 仅在 label 非 IGNORE_INDEX 的位置计算 token-level acc
            valid = labels.ne(IGNORE_INDEX)
            if valid.any():
                pred_tok = logits.argmax(-1)
                seq_tok_acc = (pred_tok.masked_select(valid) == labels.masked_select(valid)).float().mean()
            else:
                seq_tok_acc = torch.tensor(0.0, device=logits.device)

        outputs = {
            'loss': loss,  # 总损失（联合）
            'loss_seq': loss_seq.detach(),  # 便于日志跟踪
            'loss_set': loss_set.detach(),
            'seq_tok_acc': seq_tok_acc.detach(),
        ...
        if self.iter % 200 == 0:
            ...
            if ('logits' in locals()) and (logits is not None):
                ...
                print(
                    f"SeqHead Example: Pred: {example}  Ans: {example_labels}  "
                    f"loss_seq={loss_seq.item():.3f}  tok_acc={seq_tok_acc.item():.3f}"
                )
        # ---------------------------------------------------------
