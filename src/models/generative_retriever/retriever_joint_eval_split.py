...
PRIOR_SETHEAD_CKPT = "/home/iiserver31/Workbench/likaipeng/genius/checkpoint/dual_t5small-sethead-2layer-mlp/Large/Instruct/InBatch/dual_t5small-sethead-nomlp_epoch_30.pth"  # ←改成你的ckpt绝对路径
PRIOR_SCALE = 0.1  # 先导偏置强度，建议网格 0.1/0.3/0.5/0.7
from transformers.generation.logits_process import LogitsProcessor, LogitsProcessorList

class SetHeadPriorProcessor(LogitsProcessor):
    """
    用你训练好的 per-level set_head 对 z_q 打分，按“每一位的合法 code-token”给 logits 施加软偏置。
    """
    def __init__(self, model, z_q, level_token_ids, num_beams=1, prior_scale=0.3):
        super().__init__()
        self.model = model
        self.level_token_ids = level_token_ids   # list[L]，每层合法token的id（见 _build_level_token_ids）
        self.L = len(level_token_ids)
        self.prior_scale = prior_scale

        # 预计算每层先验：list[L]，形状 [B, K_l]
        with torch.no_grad():
            per_level = self.model.set_prior_logits(z_q)  # list of [B, K_l]
            if num_beams > 1:
                per_level = [lg.repeat_interleave(num_beams, dim=0) for lg in per_level]
        self.per_level_logits = per_level

    def __call__(self, input_ids, scores):
        """
        input_ids: [B*num_beams, cur_len]
        scores:    [B*num_beams, vocab_size]
        """
        # 对应当前要预测的是第几位（第 0 位开始）
        l = input_ids.size(1) - 1
        if l < 0 or l >= self.L:
            return scores

        lg = self.per_level_logits[l]  # [B*num_beams, K_l]
...
class T5ForGenerativeRetrieval(nn.Module):
    ...
    def __init__(self, config=None, tokenizer=None, clip_model=None, new_tokenizer=True, init_rq_codebook=True):
        ...
        # ---- 计算每一位的类别数 level_K（有模态位就第一位=3） ----
        self.level_K = []
        if self.modality_index:
            self.level_K.append(3)  # 首位是模态 0/1/2
        ...
        # ---- 定义 per-level MLP heads（set_head）----
        ...
        # ---- 构建“每一位对应的合法 code-token id 列表”（用于先验映射到词表）----
        self._build_level_token_ids()

        # ---- 从码本注册每一位的 prototype，做残差/归一化更稳（后面可用于 set-head 训练或 debug）----
        self._register_prototypes_from_codebooks()

        # ---- 加载你训练好的 set_head 权重（只取 set_head.*），默认冻结（先导仅做推理偏置）----
    ...
    def _register_prototypes_from_codebooks(self):
        """
        从 quantizer.residual_rq.layers 取每层码本向量，归一化后
        register_buffer 为 proto_l{idx}: [K_l, 768]
        """
        for l in range(self.codebook_level):
            E = self.quantizer.residual_rq.layers[l]._codebook.embed  # [1,K,D] 或 [K,D]
            if E.dim() == 3:
                E = E[0]
            P = F.normalize(E.float(), dim=-1)
            self.register_buffer(f"proto_l{l}", P)

    def _load_set_head_prior_hardcoded(self, freeze: bool = True):
        ...
        self.set_head.eval() if freeze else self.set_head.train()

    def set_prior_logits(self, z_q: torch.Tensor):
        """
        输入 z_q: [B,768]（量化器输出的 encode 向量，已归一化更好）
        返回 per-level logits: list[L], 每个是 [B, K_l]
        """
    ...
    def inference(self, img_emb, txt_emb, img_mask, txt_mask, inst_ids, num_beams=10, cand_codes=None):
        ...
        emb = F.normalize(emb) # ★ 建议归一化，set_head 更稳
