...
class T5ForGenerativeRetrieval(nn.Module):
    ...
    def __init__(self, config=None, tokenizer=None, clip_model=None, new_tokenizer=True, init_rq_codebook=True):
        ...
        # === 新增：优先硬编码加载你已训练好的 seq 头；加载成功就不再做码本初始化 ===
        _loaded_seq = False
        if SEQ_CKPT_PATH and os.path.exists(SEQ_CKPT_PATH):
            self._load_seq_ckpt_hardcoded()
            _loaded_seq = True

        # 若没有可用的 seq ckpt，则退回到“码本初始化”（你原来的做法）
        if (not _loaded_seq) and init_rq_codebook:
            self._initialize_codebook_embeddings(t5_config)
        ...
        _loaded_joint = self._load_joint_ckpt_hardcoded()
        # === 硬编码：只加载 set_head 先验 ===
        if getattr(getattr(self.config, "model", {}), "load_set_head_prior", False):
            self._load_set_head_prior_hardcoded(freeze=True)

        print(f"[Prior] set-head loaded (hardcoded path ok), prior_scale={PRIOR_SCALE}")



        # ---- Hard-coded Joint Model Loading (seq + set_head + others) ----
    def _load_joint_ckpt_hardcoded(self):
        path = JOINT_CKPT_PATH
        if not path or not os.path.exists(path):
            print(f"[JOINT] skip: not found -> {path}")
            return False

        sd = torch.load(path, map_location="cpu")
        sd = sd.get("model", sd)  # 兼容 {"model": state_dict}

        # 兼容DDP保存：去掉 "module." 前缀
        if any(k.startswith("module.") for k in sd.keys()):
            sd = {k[len("module."):]: v for k, v in sd.items()}

        # 词表大小可能不同：如果 ckpt 里有 embedding 权重，则先对齐尺寸
        def _maybe_resize_from_key(key):
            w = sd.get(key, None)
            if w is None:
                return
            cur = dict(self.named_parameters())
            # 有的 embedding 是 buffer/weight，要在模块上取
            target = key.split(".")[0]
            if "id_generator.shared.weight" in key:
                tgt_num = w.shape[0]
                cur_num = self.id_generator.get_input_embeddings().weight.shape[0]
                if tgt_num != cur_num:
                    self.id_generator.resize_token_embeddings(tgt_num)

        _maybe_resize_from_key("id_generator.shared.weight")
        _maybe_resize_from_key("id_generator.lm_head.weight")

        # 直接把整模型喂进来：id_generator / set_head / embed_projector ... 一次性加载
        missing, unexpected = self.load_state_dict(sd, strict=False)
        print(f"[JOINT] loaded {path} | missing={len(missing)} unexpected={len(unexpected)}")

        # 推理/继续训练都OK；此处不强制冻结，外部会 .eval()
        return True

    def _load_seq_ckpt_hardcoded(self):
        path = SEQ_CKPT_PATH
        if not path or not os.path.exists(path):
            print(f"[SEQ|hard] skip: file/dir not found -> {path}")
            return

        if SEQ_IS_HF_DIR and os.path.isdir(path):
            from transformers import T5ForConditionalGeneration
            loaded = T5ForConditionalGeneration.from_pretrained(path)
            # 词表大小若不同，先对齐一下（通常相同就不会进来）
            if loaded.get_input_embeddings().weight.shape[0] != self.id_generator.get_input_embeddings().weight.shape[
                0]:
                self.id_generator.resize_token_embeddings(loaded.get_input_embeddings().weight.shape[0])
            self.id_generator.load_state_dict(loaded.state_dict(), strict=False)
            del loaded
            print(f"[SEQ|hard] loaded HF dir: {path}")
        else:
            sd = torch.load(path, map_location="cpu")
            sd = sd.get("model", sd)  # 兼容 {"model": state_dict}
            # 既兼容“整模型state_dict”，也兼容“带前缀 id_generator.” 的state_dict
            if any(k.startswith("id_generator.") for k in sd.keys()):
                sub = {k[len("id_generator."):]: v for k, v in sd.items() if k.startswith("id_generator.")}
            else:
                sub = sd  # 假设就是 T5 权重本体
            missing, unexpected = self.id_generator.load_state_dict(sub, strict=False)
            print(f"[SEQ|hard] loaded {path} | missing={len(missing)} unexpected={len(unexpected)}")

        # 联合训练：不冻结
        for p in self.id_generator.parameters():
